...
const refeshRecipesContainer = () =>{
    ...
    recipesTab.forEach(recipe => { //C:\Users\luuta\Desktop\TP_GIT\08_TypeScript\Exercise\filter_recipes\chef-hat.svg
        const newButton = document.createElement("button");
        newButton.innerHTML = `<b>${recipe.name}</b> ${ recipe.cookTime} <i class="bi bi-fire"></i> ${recipe.prepTime}`    
        ...
        newButton.addEventListener('click', () => {
            ...
            recipesTab.forEach(recipe => {
                ...
                console.log(ingredientsList.forEach(recipe => {
                    ingredientModal.innerHTML += `<li> ${recipe.name},${recipe.amount}  </li>`
                }))
            })
        ...
        newButton.addEventListener('click', () =>{
            ...
            recipesTab.forEach(recipe =>{
                const ingredientsList = recipe.ingredients;
                console.log(ingredientsList.forEach(recipe =>{
                    instructionModal.innerHTML += `<li>${recipe.name}, ${recipe.amount}</li>`
                }));
            })
        })
